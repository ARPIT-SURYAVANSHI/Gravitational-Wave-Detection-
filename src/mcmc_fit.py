...
n_steps = 400_000              
burn_in = 100_000              
step_sizes = np.array([8e-4, 0.0010, 0.007])  
...
data_file = r"C:\Users\arpit\Downloads\jupyter notebooks\machine learning\gw_data.csv"
...
# y_std = np.std(y_data)
# y_floor = max(1e-12, 0.05 * y_std)
# y_err = np.sqrt((0.20 * np.abs(y_data))**2 + (y_floor)**2)
# print(f"Using y_err = sqrt((0.2*|y|)^2 + {y_floor:.3e}^2)  (data std {y_std:.4e}).")
...
thin = max(1, int(max(1, ess[0]//50)))  # simple heuristic: thin if ESS large
...
# fig_corner.suptitle("Posterior Distributions ", fontsize=14)
...
plt.plot(t_data, y_data, 'k.', alpha=0.5, markersize=4, label='data')
